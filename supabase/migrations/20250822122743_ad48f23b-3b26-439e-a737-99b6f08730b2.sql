-- Create overrides table and update restriction logic
...
CREATE POLICY IF NOT EXISTS "Admins can manage overrides"
...
-- Policy: users can view their own overrides (so check function in trigger context can read via invoker safely)
CREATE POLICY IF NOT EXISTS "Users can view their own overrides"
ON public.restriction_overrides
FOR SELECT
TO authenticated
...
-- Ensure dinner creation trigger still uses the updated function (recreate for safety)
