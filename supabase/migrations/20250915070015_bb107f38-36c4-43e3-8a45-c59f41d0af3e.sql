...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', usa_states.id, true, 0
FROM usa_states
JOIN (VALUES 
  ('纽约', 'NYC', 'NY'),
  ('洛杉矶', 'LA', 'CA'),
...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', uk_regions.id, true, 0
FROM uk_regions
JOIN (VALUES 
  ('伦敦', 'LON', 'ENG'),
  ('曼彻斯特', 'MAN', 'ENG'),
...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', france_regions.id, true, 0
FROM france_regions
JOIN (VALUES 
  ('巴黎', 'PAR', 'IDF'),
  ('马赛', 'MAR', 'PACA'),
...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', italy_regions.id, true, 0
FROM italy_regions
JOIN (VALUES 
  ('罗马', 'ROM', 'LAZ'),
  ('米兰', 'MIL', 'LOM'),
...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', germany_states.id, true, 0
FROM germany_states
JOIN (VALUES 
  ('柏林', 'BER', 'BE'),
  ('慕尼黑', 'MUN', 'BY'),
...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', canada_provinces.id, true, 0
...
INSERT INTO administrative_divisions (name, code, level, parent_id, is_active, display_order)
SELECT cities.name, cities.code, 'county', australia_states.id, true, 0
